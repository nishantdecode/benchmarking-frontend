...
export const baseUrl = "https://analytic.benchmarking.brihatinfotech.com/api/individualBank";
  ...
  endpoints: (builder) => ({
    ...
    exportData: builder.mutation({
      query: ({bankId, year, quarter}) => ({
        url: `/export?bankId=${bankId}&year=${year}&quarter=${quarter || ""}`,
        method: "GET",
      }),
    }),
    importData: builder.mutation({
      query: ({bankId, year, quarter, formData }) => ({
        url: `/import?bankId=${bankId}&year=${year}&quarter=${quarter || ""}`,
        method: "POST",
        body: formData,
      }),
    }),
